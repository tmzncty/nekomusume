...
fn start_server_for(
    ...
    // newline could strand the wait after the deadline check. The narrow
    // wait primitive performs the read off-thread and bounds the wait on
    // recv_timeout; on timeout/EOF/error the child is killed+reaped.
    let (child, stdout, startup_log) = wait_for_ready_marker(
        child,
        "lifecycle_state=READY readiness=true",
        Duration::from_secs(5),
    )
    .unwrap_or_else(|e| panic!("server not ready: {e}"));
    ReadyServer {
        child,
        stdout,
        startup_log,
...
/// H-I4-091: bounded server-side processing barrier — waits for `attempts`
/// `malformed_or_unadmitted` classification events from the child stdout.
/// `read_line` is blocking, so it runs off-thread; the wait is bounded by a
/// `recv_timeout` deadline and, on timeout, the child is killed and reaped so
/// the reader cannot be stranded. Returns (child, reader, collected lines)
...
/// moves the blocking `read_line` loop off-thread and bounds the wait on a
/// `recv_timeout` receiver. On every failure shape (timeout, EOF, read error,
/// channel disconnect) the owned child is killed+reaped via
/// `bounded_reap_or_kill` before the error is returned, so no failure path
/// blocks on `wait()`. On success the child, remaining-stdout reader and the
/// accumulated startup log are returned so ownership/log continuity is
/// preserved.
fn wait_for_ready_marker(
    ...
    match rx.recv_timeout(timeout) {
        Ok(Ok((stdout, startup_log))) => Ok((child, stdout, startup_log)),
        Ok(Err(startup_log)) => {
...
fn malformed_classification_barrier(
    ...
    drop(rx);
    // Do NOT join here: the reader thread is still blocked in read_line on
    // the child stdout and only returns once the child writes another line
    // (or exits). Joining now would block until the child's next output —
    // for a server that emits nothing until a later client, that strays past
    // the resource-snapshot point. Return the handle so the caller joins
...
fn ready_failover_server(child: Child) -> ReadyServer {
    // `read_line` is blocking, so the narrow wait primitive performs it
    // off-thread and bounds the wait on the receiver. On timeout/EOF/error
    // the child is killed+reaped; EOF preserves every line read before the
    // diagnostic start event.
    let (child, stdout, startup_log) = wait_for_ready_marker(
        child,
        "\"role\":\"server\",\"event\":\"start\"",
        Duration::from_secs(2),
    )
    .unwrap_or_else(|e| panic!("failover not ready: {e}"));
    ReadyServer {
        child,
        stdout,
        startup_log,
...
fn ready_endpoint_rebind_server(child: Child) -> ReadyServer {
    // I4-CLI-PROC-096: the narrow wait primitive performs the blocking
    // `read_line` off-thread and bounds the wait on recv_timeout; on
    // timeout/EOF/error the child is killed+reaped.
    let (child, stdout, startup_log) = wait_for_ready_marker(
        child,
        "endpoint_rebind_server_ready",
        Duration::from_secs(5),
    )
    .unwrap_or_else(|e| panic!("endpoint rebind not ready: {e}"));
    ReadyServer {
        child,
        stdout,
        startup_log,
...
#[test]
#[cfg(unix)]
fn ready_endpoint_rebind_bounded_when_child_closes_stdout_but_stays_alive() {
    // I4-CLI-PROC-096 negative regression: a child that closes stdout while
    // remaining alive must make the readiness wait fail within bound — the
    // EOF path must kill/reap via bounded_reap_or_kill, not block in wait().
    ...
    assert!(
        result.is_err(),
        "stdout-closing live child must make readiness wait panic, not hang"
    );
    assert!(
        elapsed < Duration::from_secs(10),
        "readiness wait must fail within bound, elapsed={elapsed:?}"
...
fn udp_listener_rejects_bounded_malformed_churn_then_authenticates_and_cleans_up() {
    ...
    // H-I4-091 ordering oracle: the post snapshot was only permitted after the
    // barrier observed ATTEMPTS malformed_or_unadmitted classifications —
    // moving it before the barrier deterministically fails this count assert.
    let malformed_classified = barrier_lines
        .matches("\"event\":\"malformed_or_unadmitted\"")
        .count();
    assert_eq!(
        malformed_classified, ATTEMPTS,
        "post snapshot requires all ATTEMPTS malformed classifications"
    );
    let (status, log) = finish_server(ReadyServer {
        child,
        stdout,
        startup_log,
        ready_proof,
...
#[test]
#[cfg(unix)]
fn malformed_classification_barrier_fails_bounded_when_events_missing() {
    // H-I4-091 negative regression: the barrier must fail deterministically
    // within its bounded deadline when the server does not emit the required
    // malformed_or_unadmitted classification count, and the child must be
    // reaped so the reader cannot be stranded.
    let _port_lock = TEST_PORT_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let bin = env!("CARGO_BIN_EXE_neko-cli");
    let sp = tmp("preauth-churn-server");
    let cp = tmp("preauth-churn-client");
    ...
    let server = Command::new(bin)
        ...
        .unwrap();
    let server = ready_failover_server(server);
    // Send ZERO malformed datagrams, then require 1 classification — the
    // barrier must fail within its bounded deadline, not hang.
    let ReadyServer { child, stdout, .. } = server;
    let start = std::time::Instant::now();
    let result = malformed_classification_barrier(child, stdout, 1, Duration::from_secs(2));
    let elapsed = start.elapsed();
    assert!(
        result.is_err(),
        "barrier must fail without classification events"
    );
    assert!(
