...
| `close_after: N` (3d2f960) | at send index N the seam CLOSES the carrier — a visible transport-close transition distinct from blackhole's silent drop: the triggering send returns `Err(Closed)` (UdpLoopback/Tcp close semantics), subsequent sends return Closed idempotently, queued records drain via recv then None, and a withheld reorder record is discarded with the connection | sender observes an explicit close error (fail-closed signal), not silent loss | `close_after_is_a_visible_transport_close` (clean-close drain + idempotence + withheld-record discard) | close is terminal at the seam (no half-open interval); recv draining is the inner carrier's existing queue semantics |
