...
| Primitive | Deterministic behavior | Session/state expectation | Proven by | Unresolved boundary |
|---|---|---|---|---|
| `reorder` (R-MBOX-REORDER-DELAY) | bounded one-record pending slot withholds one surviving record; the NEXT surviving record releases it after itself (adjacent-pair swap). **Swap atomicity (c389ec6):** if the inner carrier rejects the second half of a swap (BufferFull/Closed/PeerClosed), the withheld record is restored to the slot and the error propagates — a swap delivers both halves or retains the record for retry, never a silent loss | receiver observes pairwise-swapped delivery order; ordering assumptions are challenged without loss | `reorder_swaps_adjacent_pairs_and_is_bounded`; `reorder_swap_failure_restores_pending_atomically`; interaction tests with loss/MTU/window above | bounded to one withheld record by construction (`pending_len` 0/1); only adjacent-pair swaps, no wider permutation model; no reorder-impact-on-Session-queue claim (Session-level reorder tolerance is separately covered by existing SessionRuntime order-independent DeliveryAck semantics) |
