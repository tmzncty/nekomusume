...
- Security promotion also remains blocked by RSEC-001 in `docs/reviews/resource-abuse-evidence-2026-09-04.md`: process-owned source/global pre-auth accounting state now exists with deterministic tests, but it is not integrated at each TCP/UDP listener boundary and its source-tuple/charge instrumentation is not reviewed.
