...
| session-model | in-memory Session delivery state | candidate | `crates/neko-session/src/lib.rs` | Candidate model; bounded ledger question reviewed, complete Session/release validation not established |
...
- NAT / source-endpoint change — `ALREADY_SUFFICIENT_FOR_BOUNDED_QUESTION`: exact `a8f49fc` repairs the authenticated promotion barrier and candidate semantics, has green exact-head CI, and provides one fresh bounded self-owned source-port-change observation in `docs/notes/endpoint-rebind-vps-a8f49fc-20260908.md`. It proves one same-Session carrier/source-binding generation change under a fixed crypto record context only—not general NAT traversal, cryptographic path-generation migration, roaming reliability, public reachability or production readiness. Exact `c81fa73` remains superseded candidate provenance.
...
- HY2 fair pair — `BLOCKED_HARNESS_CURRENT_LINE`: exact `13da094` consumed exactly one materially changed self-owned attempt after green CI. The valid prefix is `nekomusume-1` success followed by `hy2-1` `client_exit`, zero HY2 application bytes, typed diagnostic `unknown`, baseline `last_success_stage=client_started` / `last_success_source=harness`, and verified zero-residue cleanup. Result SHA-256: `dffab760a660b3ce9addcbd13bedb15f0b0a8f58624088da000c178de0de14cb`. There is no complete pair, summary, or performance claim; no same-class retry is permitted without a new material hypothesis. Evidence: `artifacts/hy2-owned-lab/13da094-hy2-diagnostic-attempt/`.
- repeated warm failover — `BLOCKED_ORCHESTRATION_CURRENT_LINE`: exact `f17b648`, after the final startup-poll attribution and cleanup-precedence repairs plus green exact-head CI, consumed exactly one post-fix self-owned outer invocation. It stopped before cycle 1 with `completed_cycles=0`, `invalid_cycle_evidence`, and primary `diagnostic_category=startup_setup` (sanitized diagnostic SHA-256 `5867711ad01f5cc505e982f9b5956b4b05d08758c2071f4fe6d5a6edaec4d101`, 160 bytes). A separate serialized post-run check found zero listeners/processes and the deployment was removed. This repeats the same evidence boundary as exact `9a94922`; it is not a root cause, no cycle/runtime/failover conclusion follows, and no same-class retry is permitted without a new material hypothesis. Evidence: `docs/notes/repeated-warm-failover-f17b648-postfix-negative.md`. Exact `2b784a3`/`3aa4828` and `9a94922` remain unchanged historical evidence.

There is no remaining unimplemented `READY_LIVE` row; live key update, migration-back, and the exact-`a8f49fc` bounded same-Session source-port-change row are sufficient for their declared self-owned questions. The repeated-failover diagnostic seam is complete; its current same-class WAN line is frozen unless a concrete material setup hypothesis emerges. The bounded release matrix therefore remains open, with
...
- `ALREADY_SUFFICIENT_FOR_BOUNDED_QUESTION`: A, B, C, D, E, F, G, H, I, J, K, L, M, N, O, T. B is limited to the exact-`525b61b` frozen 42-vector/10-domain corpus, real implementation oracles, candidate wire/compatibility tests, and isolated decode fuzz smoke; it does not establish previous/current interoperability because no prior frozen release exists. C is limited to exact-`531e0bc` bounded candidate state/evidence semantics: queued `Unsent -> InFlight` bytes retain assignment-time context until explicit confirmation, while advanced duplicate/overlap rollback and confirmation/watermark atomicity remain tested; it is not complete Session validation. K is limited to exact `5d6582c` scripted migration-back evidence; N is limited to the existing nine-scenario privileged ICMP/netem harness and is not Nekomusume performance; O aggregates only already-answered bounded self-owned VPS questions and does not create a new live opportunity. Do not re-run these rows merely because broader testing is possible.
- `OPEN_READY`: none. A row reopens only for a newly demonstrated concrete contradiction or missing named failure mode; this does not change policy/environment/governance-blocked rows. Each carries a specific unresolved question, `evidence_needed`, `next_action`, dependency statement, and local execution scope; this is an opportunity classification, not authorization or a release claim.
- `BLOCKED_DEPENDENCY`: none after K/N/O current-fact reconciliation. Removing an obsolete dependency explanation does not promote P/Q/S/U/V/W through their independent governance/orchestration/implementation gates.
- `BLOCKED_IMPLEMENTATION`: R (PLPMTUD integration). NAT/source-endpoint change is `ALREADY_SUFFICIENT_FOR_BOUNDED_QUESTION` at exact `a8f49fc` for one same-Session source-port observation under a fixed crypto record context; exact `c81fa73` remains superseded candidate evidence; K (migration-back) is `ALREADY_SUFFICIENT_FOR_BOUNDED_QUESTION` at exact `5d6582c`; live key update is `ALREADY_SUFFICIENT_FOR_BOUNDED_QUESTION` at exact `2f4f59a`/`69d0ed9`.
- RSEC-001 — `ENGINEERING_CONTROLS_REVIEWED / SOURCE_RETENTION_POLICY_BLOCKED`: exact `6952fa9` review confirms non-policy source tuple projection, charge-before-parse and exact-response charging, all 7 inventoried responder surfaces / 6 admission sites, sibling-scoped pending-owner proof, atomic rollback/terminalization, source/global concurrency, expiry/release/error cleanup, bounded response deadlines and aggregate-only diagnostics. Adversarial-load suitability remains open. D019 terminal source retention after the last live state remains a maintainer/security-policy checkpoint; the release/reopen test demonstrates that conflict and is not compliance. RSEC-001 is not closed; public-listener, RC, production and release claims remain blocked.
