...
- NAT / source-endpoint change — `BLOCKED_IMPLEMENTATION`: exact `c81fa73` supplied local/CI real-socket direction, but reviewer EPREB-006/007 found the missing authenticated promotion synchronization barrier and an unsolicited D064 response sentinel. The retained `docs/notes/endpoint-rebind-vps-c81fa73-20260908.md` is superseded candidate evidence and is not accepted for the release matrix; rerun only after the barrier repair and fresh exact-head gates.
...
There is no remaining unimplemented `READY_LIVE` row; live key update and migration-back are sufficient for their declared self-owned questions. NAT/source-endpoint remains implementation-blocked pending EPREB-006/007 barrier repair. The next repeated-failover seam, if selected, is local sanitized inner-collector failure categorization, not another runner-entry retry. The bounded release matrix therefore remains open, with
...
- `BLOCKED_IMPLEMENTATION`: R (PLPMTUD integration) and NAT/source-endpoint change pending EPREB-006/007 barrier repair; exact `c81fa73` is superseded candidate evidence only; K (migration-back) is `ALREADY_SUFFICIENT_FOR_BOUNDED_QUESTION` at exact `5d6582c`; live key update is `ALREADY_SUFFICIENT_FOR_BOUNDED_QUESTION` at exact `2f4f59a`/`69d0ed9`.
