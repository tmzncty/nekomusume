...
| cli | bounded authenticated probe CLI | candidate | `crates/neko-cli/src/main.rs` | Generic TCP/UDP probes perform canonical bounded version negotiation before Noise, bind the exact transcript into the authenticated prologue, and admit echo data only after negotiation plus authentication; fixed 40080-40100 ports, 1-1200 bytes, 1-30s duration; no failover/resume negotiation, proxy/tunnel, or production listener |
