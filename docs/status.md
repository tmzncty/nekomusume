...
- `ALREADY_SUFFICIENT_FOR_BOUNDED_QUESTION`: A, B, G, H, I, J, K, N, O. B is limited to the exact-`525b61b` frozen 42-vector/10-domain corpus, real implementation oracles, candidate wire/compatibility tests, and isolated decode fuzz smoke; it does not establish previous/current interoperability because no prior frozen release exists. K is limited to exact `5d6582c` scripted migration-back evidence; N is limited to the existing nine-scenario privileged ICMP/netem harness and is not Nekomusume performance; O aggregates only already-answered bounded self-owned VPS questions and does not create a new live opportunity. Do not re-run these rows merely because broader testing is possible.
- `OPEN_READY`: C, D, E, F, L, M, T. Each carries a specific unresolved question, `evidence_needed`, `next_action`, dependency statement, and local execution scope; this is an opportunity classification, not authorization or a release claim.
