...
The smoke validates the complete expected archive shape before extraction, rejects absolute/traversal paths and non-regular members (including links and special files), extracts without archive ownership or permission inheritance, verifies `SHA256SUMS`, requires mode `0755` for the executable and `0644` for documentation, then runs secret-free `capabilities --json` when target equals host. Synthetic adversarial archive regressions are wired into `scripts/check.sh`. On Unix, the CLI creates identities atomically as new regular files with exact mode `0600` before private bytes are written, independent of caller umask; existing identities are opened with no-follow/close-on-exec flags and validated/read through the same descriptor. Reload accepts only regular owner-only files and rejects insecure paths without modifying them; the advertised multistream command now reuses the same descriptor-bound existing-identity reader and continues to require an existing identity rather than generating one. Exact multistream path/config validation is retained in [`local-multistream-identity-1648dfd-20260909.md`](local-multistream-identity-1648dfd-20260909.md). Server/client, failover/endpoint-rebind, and periodic server/client deterministic trust, bind/address, and payload configuration is validated before automatic identity creation, so such rejected commands leave a nonexistent identity path absent; network bind/connect failure remains a separate runtime boundary. The cross-command ordering repair and focused ordinary/failover/endpoint-rebind rejection matrix are anchored by exact `aefd49f` developer-local validation. The canonical `failover --role server|client` entry point reuses the existing bounded failover implementations; its positive loopback route and packaged invalid-role side-effect check are retained in [`local-canonical-failover-f2d2e8c-20260909.md`](local-canonical-failover-f2d2e8c-20260909.md). Operators must never package, copy, log, or overwrite an existing identity. Exact-tree path validation is retained in [`local-identity-security-0e664d6-20260909.md`](local-identity-security-0e664d6-20260909.md); the prior native packaged smoke remains in [`local-identity-security-749976f-20260909.md`](local-identity-security-749976f-20260909.md).
