...
The smoke validates the complete expected archive shape before extraction, rejects absolute/traversal paths and non-regular members (including links and special files), extracts without archive ownership or permission inheritance, verifies `SHA256SUMS`, requires mode `0755` for the executable and `0644` for documentation, then runs secret-free `capabilities --json` when target equals host. Synthetic adversarial archive regressions are wired into `scripts/check.sh`. On Unix, the CLI creates identities atomically as new regular files with mode `0600`; reload accepts only regular non-symlink files with no group/world permission bits and rejects insecure existing paths without modifying them. Operators must never package, copy, log, or overwrite an existing identity. Exact-tree and native packaged evidence is retained in [`local-identity-security-749976f-20260909.md`](local-identity-security-749976f-20260909.md).
