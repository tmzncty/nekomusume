# Release item-4 subgate factual review — exact `20396f9`

## Scope

Developer-prepared factual review support for reachable exact commit `20396f91918342f34e94654edacb74bc310f5152`. This is not an independent review, security approval, release decision, protocol freeze, RC, public-listener approval, or production authorization. Release flags remain false.
...
On reachable exact `20396f9`, developer-local `PYTHONDONTWRITEBYTECODE=1 bash scripts/check.sh`, `git diff --check`, and clean initial/final tree checks passed. The local process fixtures hold TCP+UDP port leases through selection and release them at child-spawn handoff, narrowing the prior unbounded selection/use gap without changing the operator port range; provenance is retained in [`../local-test-port-lease-20396f9-20260910.md`](../local-test-port-lease-20396f9-20260910.md). On prior exact `7f0b9a3`, Periodic server/client deterministic peer/address/bind rejection now precedes persistent identity creation, the complete focused matrix is retained, and the earlier fixed-port failover gate failure is replaced by test-only available-port selection inside the unchanged operator range; provenance is retained in [`../local-periodic-identity-7f0b9a3-20260910.md`](../local-periodic-identity-7f0b9a3-20260910.md). Exact `363177c` remains unchanged historical red-gate evidence. On prior exact `1648dfd`, The advertised multistream command now reuses the descriptor-bound existing-identity reader, rejects insecure/symlink paths and deterministic config before network effects, and preserves its authenticated positive path; provenance is retained in [`../local-multistream-identity-1648dfd-20260909.md`](../local-multistream-identity-1648dfd-20260909.md). On prior exact `f2d2e8c`, native x86_64 package build/smoke and the canonical failover bounded controlled UDP-to-TCP loopback passed. The canonical `failover --role server|client` path completed the bounded controlled UDP-to-TCP loopback success; provenance is retained in [`../local-canonical-failover-f2d2e8c-20260909.md`](../local-canonical-failover-f2d2e8c-20260909.md). Exact `aefd49f` remains the preceding cross-command identity/config matrix evidence. Its table-driven process regression covers ordinary, failover, and endpoint-rebind peer/address/bind rejection before identity creation. Provenance is retained in [`../local-identity-config-aefd49f-20260909.md`](../local-identity-config-aefd49f-20260909.md). Exact `489c345` remains the prior ordinary-command/umask gate, exact `0e664d6` the descriptor-bound reload gate, and hosted runs remain separate evidence. A green gate is not a release/security decision.
