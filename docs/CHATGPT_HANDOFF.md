...
- **H-I4-101 is OPEN/HIGH** because the newly introduced bounded exit helper and the existing `finish_server` still abandon unresolved child ownership on `try_wait()` error; details and closure contract are in `docs/reviews/reviewer-h-i4-101-bounded-wait-try-wait-error-ownership-20260922.md`.
