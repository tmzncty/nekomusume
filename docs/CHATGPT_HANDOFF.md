...
- **H-I4-107/108/109 are CLOSED** at `4c445ce36aa06edc1b3d4952036073865e3f20d0`: `bounded_wait_with_output` (probe.rs) `kill()` `Err` and post-kill `try_wait` `Err` are explicit fail-closed panics; `bounded_client_output` bounds `run_client`/`authenticated`/`failover-client` owners with caller timeout `--duration + 5s`; `multistream.rs` `bounded_wait_with_output` and `bounded_client_output`/`multistream_client` are retained from H-I4-107. Exact-tree provenance covering all three: `docs/notes/h-i4-107-108-109-provenance-4c445ce-20260923.md` — `check.sh` exit 0, `git diff --check` exit 0, clean worktree, 2026-09-23T03:02:29Z → 03:08:20Z, Linux x86_64, rustc 1.98.0.
