...
- **H-I4-113 and H-I4-114 are CLOSED** at `46d624e2272fcd5cf9d86457d018c4f508b8ba10`: seven periodic-client owners (`--duration 5→10s`, `--duration 2→7s`), two endpoint-rebind-client owners (`--duration 5→10s`, `--duration 3→8s`), matrix-probe's run closure (`probe --matrix`, 10s), and duplicate-negotiation TCP `set_read_timeout` before first `frame_read_test`. Exact-tree provenance covering both: `docs/notes/h-i4-113-114-provenance-46d624e-20260923.md` — `check.sh` exit 0 (first run transient UDP bind race, rerun clean), `git diff --check` exit 0, clean worktree, 2026-09-23T14:51:06Z → 14:56:50Z, Linux x86_64, rustc 1.98.0.
