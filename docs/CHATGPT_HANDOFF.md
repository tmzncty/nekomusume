...
- **H-I4-095 is CLOSED** at `0ed814ce4adbc6de9b803904baa665629dc2bb15`: `ReadyProof` and `BarrierProof` are target-neutral zero-sized markers (no Unix API), so the `#[cfg(unix)]` attribute was removed — non-Unix builds now compile `probe.rs` without a missing-type/name-resolution failure while the Linux `/proc` causal coupling stays intact. Exact-tree provenance: `docs/notes/h-i4-095-provenance-0ed814c-20260922.md` — `check.sh` exit 0, `git diff --check` exit 0, clean worktree, 2026-09-22T05:50:18Z → 05:56:15Z, Linux x86_64, rustc 1.98.0.
