...
- **H-I4-085 remains OPEN / HIGH (release-item-4 dependency/build evidence truth).** The original direct-dependency contradiction was partially repaired: exact `c4925b67eb3f8425c77e53076248ab8e569f2d6e` now correctly classifies `snow` as a normal production dependency of `neko-crypto`, and exact `803bee32c40a676612e89c9616797c600453eb19` records locked Cargo-tree evidence. However, the same I4-BLD note still claims that build/native hooks are absent and reports `cargo tree --locked -e build -p neko-crypto` as having no build dependencies. That conclusion is false for the active resolved production closure: `snow 0.10.0` has a build script plus `rustc_version`, and its default-feature normal graph activates `ring`; resolved `ring 0.17.14` has `build.rs`, `links = "ring_core_0_17_14_"`, and a `cc` build dependency/native build surface. The bounded continuation finding and closure contract are in `docs/reviews/reviewer-h-i4-085-build-hook-reopen-20260921.md` at exact `a7ea381`.
