...
- **H-I4-090 remains OPEN as HIGH evidence-oracle closure.** Independent review at `docs/reviews/h-i4-090-phase-sentinel-gap-20260921.md` found that `churn_phase` is changed to `2` only after every malformed `send_to` has completed. Moving the `before` snapshot block to immediately after the send loop but before `churn_phase = 2` still sees phase `0`, so the regression mutation that originally caused the false-negative can remain green. The committed runtime/test order is correct; the mutation guard is not yet strong enough.
