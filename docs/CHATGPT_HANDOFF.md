...
- **H-I4-089's portability correction remains valid and closed** at `26a2c40`: `process_resource_snapshot` is Linux-only; Linux observation must be affirmative; non-Linux Unix retains the portable socket/lifecycle portion without fabricating `/proc` evidence.
- **H-I4-090 is CLOSED** at `0dbb93145c4e1cc031ca30258b3c58a5c3155b51`: the pre-churn `before` baseline is sampled **before** the malformed senders fire — a post-churn baseline cannot prove bounded growth. The `after` snapshot and FD/RSS assertions remain affirmative on Linux. Exact-tree provenance: `docs/notes/h-i4-090-provenance-0dbb931-20260921.md` — `check.sh` exit 0, `git diff --check` exit 0, clean worktree, 2026-09-21T08:49:56Z → 08:55:50Z, Linux x86_64, rustc 1.98.0.
