...
- **H-I4-092 is CLOSED** at `7f678fcac75233a1eac5e59875d56e12941e15cf`: `gated_resource_snapshot(gate, pid)` runs the phase assert inside the same call as `process_resource_snapshot` — splitting the assert from the measurement breaks compilation, so a refactor cannot leave a post-churn sample with a pre-churn assert (pre-churn edge) or a pre-barrier sample with a post-barrier assert (post-barrier edge). Both snapshots in `udp_listener_rejects_bounded_malformed_churn_then_authenticates_and_cleans_up` use the coupled helper. Exact-tree provenance: `docs/notes/h-i4-092-provenance-7f678fc-20260922.md` — `check.sh` exit 0, `git diff --check` exit 0, clean worktree, 2026-09-22T01:50:34Z → ~01:57:00Z, Linux x86_64, rustc 1.98.0.
