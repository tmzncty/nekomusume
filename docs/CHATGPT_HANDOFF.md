...
- **H-I4-091 is CLOSED** at `a5e8e7665041027234fd5d6ea8f125ddefe51bbc` (docs-manifest) on executable/test owner `2a36892a958028ad3ce9c8ddd2e79e7c6cf68dec`: the bounded post-release barrier emits `malformed_or_unadmitted` AFTER `preauth.release(admission)`, with an off-thread `read_line` + `recv_timeout` bounded wait, child kill+reap on timeout, and a `malformed_classification_barrier_fails_bounded_when_events_missing` negative regression proving bounded failure on missing classifications. The `churn_started` sentinel makes moving the baseline into the post-churn window a hard failure; the post snapshot is taken after the barrier proves all `ATTEMPTS` malformed datagrams were classified. Exact-tree provenance: `docs/notes/h-i4-091-provenance-a5e8e76-20260921.md` — `check.sh` exit 0, `git diff --check` exit 0, clean worktree, 2026-09-21T23:03:35Z → 23:09:15Z, Linux x86_64, rustc 1.98.0.
