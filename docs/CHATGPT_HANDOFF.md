...
- **H-I4-092 is REOPENED** by reviewer finding `de67348232604da2f32ca34b7f03a8ac1958ca9f` (`docs/reviews/h-i4-092-post-barrier-proof-gap-20260922.md`): the post edge still uses a freely constructed `barrier_complete = true` plus a generic `FnOnce()` gate. That value is not returned/derived from successful `malformed_classification_barrier(...)`, so moving both the boolean and coupled snapshot above the barrier (or supplying an always-successful gate) still compiles and can silently turn the post sample into a pre-barrier sample.
