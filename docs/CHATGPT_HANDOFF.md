...
- **H-R9-079 is OPEN HIGH:** `SessionRuntime::clear_runtime_state()` leaves `streams` and `close_deadline_ms` alive after terminalization even though they are mutable live-session stream/timer owners and are not among the explicitly documented lifetime/cumulative survivors. Existing tests named `*_releases_all_runtime_owned_state` do not challenge those two owners.
