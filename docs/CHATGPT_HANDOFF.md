...
- **H-I4-111 is only PARTIALLY CLOSED.** Exact `1d727f0` still leaves both real clients in `expired_preprogress_udp_session_is_retired_before_delivery_and_fresh_handshake_recovers` as direct synchronous `.output().unwrap()` owners before `finish_server(server)` becomes reachable. `docs/notes/h-i4-111-provenance-1d727f0-20260923.md` falsely states that those two clients already use `bounded_client_output`. Reviewer finding: `docs/reviews/h-i4-112-h-i4-111-provenance-source-mismatch-20260923.md`.
