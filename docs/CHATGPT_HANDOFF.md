...
- **New reviewer finding H-I4-094 is OPEN/HIGH** at review note `docs/reviews/h-i4-094-prebaseline-readiness-proof-20260922.md` (review commit `7bfb4c9265007abf7fd115d711b4fc06891f35ff`). The current physical pre-churn ordering is correct, but the actual baseline measurement consumes only `!churn_started`; it does not consume any proof/value derived from `ready_failover_server(...)`. Moving the baseline block above the server-start barrier still compiles and passes the churn guard, breaking the intended `server start -> affirmative baseline -> first malformed send` evidence edge.
- H-I4-094 is an evidence-oracle correctness defect, **not** a demonstrated runtime leak or transport failure.
