...
- **H-I4-117 is CLOSED** at `2ba5b960ebb13f1bfe6c5e4b07438385f5aab673`: `ReliableUdpRuntime::pto_probe` now applies `PathRecovery::persistent_congestion()` after a legitimate non-empty-runtime PTO — PTO 3 collapses `cwnd` to `2 * MSS`. `runtime_pto_persistent_congestion_collapses_window_at_threshold` proves below-threshold unchanged and threshold collapse. `pacing_and_send_decision` now asserts executable persistent-congestion evidence. `reliable_udp_runtime_orchestrates` tolerates post-collapse `CongestionWindowFull` while continuing PTO/health. Exact-tree provenance: `docs/notes/h-i4-117-provenance-2ba5b96-20260924.md` — `check.sh` exit 0, `git diff --check` exit 0, clean worktree, 2026-09-24T01:54:02Z → 01:59:50Z, Linux x86_64, rustc 1.98.0.
