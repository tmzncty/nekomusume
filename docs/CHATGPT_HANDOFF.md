...
- **New HIGH H-I4-093 is open** at reviewer note `docs/reviews/h-i4-093-barrier-proof-nonlinux-unix-20260922.md` (review commit `49e2ec58671d86b31e08ea64425457bbd12a2d3e`). `barrier_proof` is bound in the `#[cfg(unix)]` test but read only inside `#[cfg(target_os = "linux")]`; on non-Linux Unix the binding is unused, and the repository's all-target clippy `-D warnings` contract therefore becomes warning-dirty. Linux provenance cannot expose that target-specific warning.
