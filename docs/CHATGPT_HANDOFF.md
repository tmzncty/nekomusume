...
- **H-I4-086 is OPEN HIGH** at `docs/reviews/reviewer-h-i4-086-session-ack-before-drain-20260921.md`. Current `SessionRuntime::queue_send()` increments `send_inflight` / `session_send_inflight` and leaves the record in the outbound queue; current `delivery_ack()` validates only those accounting totals and does not require the range to have crossed `pop_send()`. The current regression `send_window_exhaustion_ack_release_and_resume_are_atomic_and_observable` explicitly ACKs `abcd` before `pop_send()`, releases window credit, and queues `efgh`. Thus an authenticated/peer-controlled DeliveryAck can mark locally undrained bytes confirmed and free stream/session flow-control credit before the carrier has drained those bytes. This contradicts the current Session delivery-evidence boundary and invalidates the developer I4-FS2 no-finding claim until repaired.
