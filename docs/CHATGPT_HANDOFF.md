...
- **H-R9-083 is CLOSED** at exact `1dbe1543e5b82458c90a866cc5f0fc8c9bbd4a24`: `owned_port_sockets_present()` now scans TCP/TCP6 LISTEN and UDP/UDP6 bound sockets for supplied ports — matching the sampler's resource accounting surface. It returns `True` (present), `False` (absent), or `None` (unknown — incomplete `/proc/net` observation never promoted to success); OSError no longer silently yields `False`. `cleanup.complete` requires `group_empty` AND an affirmative absent. `escu.py` spawns a `setsid` descendant holding a bound UDP socket on the owned port; sampler reports `process_group_empty=true` but `owned_sockets_after_exit=None` and `complete=false`. Developer-local clean exact-tree provenance for exact `1dbe154`: `PYTHONDONTWRITEBYTECODE=1 bash scripts/check.sh` exit 0, `git diff --check` exit 0, clean worktree, 2026-09-20T09:51:03Z → 2026-09-20T09:57:13Z, Linux x86_64, rustc 1.98.0.
