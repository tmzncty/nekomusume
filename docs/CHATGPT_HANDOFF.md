...
- **H-I4-118 is CLOSED** at `71b62c642a6ea736fd162653ede9b29868e08eee`: `Recovery::on_ack` loss candidates are now bounded by `n <= largest` — a legal duplicate/stale ACK for a retired older packet can no longer fabricate `lost_packets`/`retransmit_frames`/congestion evidence on newer in-flight packets. `stale_ack_does_not_time_threshold_newer_packets` proves the invariant. Exact-tree provenance: `docs/notes/h-i4-118-provenance-71b62c6-20260924.md` — `check.sh` exit 0, `git diff --check` exit 0, clean worktree, 2026-09-24T04:50:06Z → 04:56:00Z, Linux x86_64, rustc 1.98.0.
