...
- **H-I4-097 is CLOSED** at `76f33be72a1c3c24152193fb174194631cb7cf95`: `bounded_reap_or_kill` no longer falls through to an unbounded `wait()` on `try_wait` `Err`, and `kill` `Err` while the child may remain live is an explicit panic — not a license to block. The post-kill reap is a bounded `try_wait` poll loop (3s deadline + 10ms poll). `malformed_classification_barrier`'s timeout/EOF/channel-error exits share the same bounded cleanup rule. Exact-tree provenance: `docs/notes/h-i4-097-provenance-76f33be-20260922.md` — `check.sh` exit 0, `git diff --check` exit 0, clean worktree, 2026-09-22T09:51:13Z → 09:57:05Z, Linux x86_64, rustc 1.98.0.
...
- **I4-CLI-PROC-096 is re-closed by H-I4-097's `bounded_reap_or_kill` repair:** `b4af007`'s off-thread readiness reads and bounded `recv_timeout` waits remain intact, and the shared `bounded_reap_or_kill` now panics on `try_wait`/`kill` failure rather than blocking in `wait()`. The `49925ac` no-finding is restored on this invariant.
- **I4-BND at `5b7b22b` is re-closed by H-I4-097:** the `sync_channel(64) -> sync_channel(1)` repair remains accepted, and all timeout/EOF/disconnect cleanup paths now converge through the same bounded `bounded_reap_or_kill`/`try_wait`-poll rule.
