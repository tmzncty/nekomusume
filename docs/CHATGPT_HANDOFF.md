...
- **H-I4-088 is OPEN/HIGH.** Exact-current `MemoryEndpoint::send` bounds a peer queue only by `queue_bytes + message.len() <= max_queue_bytes`, while empty messages are an explicitly supported behavior and still append a `Vec`/`VecDeque` entry. Repeated `send(&[])` therefore grows queue record/memory ownership without increasing `queue_bytes`; the configured/reported `max_buffered_bytes` does not bound this valid message shape. Independent finding: `docs/reviews/reviewer-h-i4-088-memorycarrier-empty-message-unbounded-20260921.md`.
