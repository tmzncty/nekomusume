...
- [x] **Distinct-version package rehearsal:** exact old `91a735c` package → exact current-at-run `dc90c5f` package → old package completed one authenticated 32-byte TCP exchange at each stage, with differing archive/binary hashes, external state marker retention, final rollback to A, and verified cleanup. This is bounded operator evidence, not arbitrary state-schema compatibility or release approval.
- [x] **Package archive validation:** smoke now fails closed before extraction on path escape, links/special members, unexpected roots/layout, checksum mismatch and insecure modes; synthetic adversarial archive regressions run in `scripts/check.sh`. Signing, key custody, SBOM and publication trust remain separate.
...
3. [ ] **Bounded release evidence matrix.** Under standing authorization, collect
   ...
   The historical exact-`60cd40d` periodic pre-application negative remains retained. Exact `85346ce` retains the sole exact-`00ac2c1` changed-hypothesis follow-up: `ssh_transport_exit` 255, no readiness/client/application metrics, and verified-zero cleanup. Its R-009 erratum corrects the legacy `protocol_entered=true` interpretation without rewriting the artifact. The periodic line is `BLOCKED_ORCHESTRATION_CURRENT_LINE_PERIODIC`; exact `4a2129e` keeps repeated failover `BLOCKED_ORCHESTRATION_CURRENT_LINE`. Natural loss is unchecked; exact `a8f49fc` provides one bounded authenticated same-Session source-port-change observation with post-promotion synchronization under a fixed crypto record context (general NAT/public reachability remains blocked), while exact `c81fa73` remains superseded candidate evidence; live PMTUD remains implementation-blocked; migration-back is `ALREADY_SUFFICIENT_FOR_BOUNDED_QUESTION` at exact `5d6582c` with one scripted self-owned observation; live key update is bounded-live sufficient at exact 2f4f59a/69d0ed9; IPv6 remains environment-blocked; the historical exact-`61a6490` HY2 line closed at local port-range preflight, while the current exact-`13da094` line is `BLOCKED_HARNESS_CURRENT_LINE_HY2` at typed `unknown` / `client_started` after one changed-hypothesis attempt, with no complete pair or same-class retry. This release-evidence item stays open.
...
- `ALREADY_SUFFICIENT_FOR_BOUNDED_QUESTION`: NAT/source-endpoint change at exact `a8f49fc` for one authenticated same-Session source-port-change observation with a post-promotion sync barrier under a fixed crypto record context; exact `c81fa73` remains superseded candidate evidence and general NAT/public reachability remains outside the claim.
...
- `BLOCKED_HARNESS_CURRENT_LINE_HY2`: exact `13da094` consumed exactly one materially changed self-owned attempt after green CI. It retained a valid `nekomusume-1` success followed by `hy2-1` `client_exit`, zero HY2 application bytes, typed diagnostic `unknown`, baseline `last_success_stage=client_started` / `last_success_source=harness`, and verified zero-residue cleanup. Result SHA-256: `dffab760a660b3ce9addcbd13bedb15f0b0a8f58624088da000c178de0de14cb`. No complete pair or performance claim follows; same-class retry is frozen without a new material hypothesis. Earlier `61a6490` and `3d54585` evidence remains immutable.
- `BLOCKED_ORCHESTRATION_CURRENT_LINE_HY2`: historical exact `61a6490` C follow-up stopped at local port-range preflight (exit 2), with zero VPS deployments/samples/result/comparative statistics/runtime evidence; historical bed2940 is separate. Both historical lines verified zero cleanup and did not touch existing Hysteria service.
