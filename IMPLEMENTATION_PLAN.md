...
3. [ ] **Bounded release evidence matrix.** Under standing authorization, collect
   ...
   remain classified as such. The D064 pre-failure seam now uses three bounded authenticated peer request/response observations, fail-closed responder admission, one-second server readiness and remaining-duration I/O deadlines, deterministic reset-on-failure manager policy, live runtime admission, negative process tests, and local warm/cold tests, but current-exact-head VPS warm recovery remains absent: the retained changed-path run failed closed at challenge 3. A current-tree periodic row also remains absent because the server authenticated but the client failed to receive the handshake response. The owned-lab fair-pair adapter and exact-payload seam are implemented and fully gated, but a temporary HY2 QUIC/UDP path timeout prevented all comparative samples, statistics and superiority claims. IPv6 remains environment-blocked, and the remaining matrix rows are still open; this item is
