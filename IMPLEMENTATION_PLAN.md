...
3. [ ] **Bounded release evidence matrix.** Under standing authorization, collect
   ...
   The historical exact-`60cd40d` periodic pre-application negative remains retained. Exact `85346ce` retains the sole exact-`00ac2c1` changed-hypothesis follow-up: `ssh_transport_exit` 255, no readiness/client/application metrics, and verified-zero cleanup. Its R-009 erratum corrects the legacy `protocol_entered=true` interpretation without rewriting the artifact. The periodic line is `BLOCKED_ORCHESTRATION_CURRENT_LINE_PERIODIC`; exact `4a2129e` keeps repeated failover `BLOCKED_ORCHESTRATION_CURRENT_LINE`. Natural loss is unchecked; exact `c81fa73` now provides one bounded authenticated same-Session source-port-change observation (general NAT/public reachability remains blocked); live PMTUD remains implementation-blocked; migration-back is `ALREADY_SUFFICIENT_FOR_BOUNDED_QUESTION` at exact `5d6582c` with one scripted self-owned observation; live key update is bounded-live sufficient at exact 2f4f59a/69d0ed9; IPv6 remains environment-blocked; the current HY2 line is closed as `BLOCKED_ORCHESTRATION_CURRENT_LINE_HY2` after exact-`61a6490` local port-range preflight, with no unchanged retry. This release-evidence item stays open.
...
- `ALREADY_SUFFICIENT_FOR_BOUNDED_QUESTION`: NAT/source-endpoint change at exact `c81fa73` for one authenticated same-Session source-port-change observation; general NAT/public reachability remains outside the claim.
