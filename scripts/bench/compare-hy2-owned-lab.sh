...
SETUP_SEC=30; READINESS_SEC=10; DIAGNOSTIC_SEC=20; CLEANUP_SEC=30
WHOLE_LAB_SEC=$((RUNS * 2 * (TIMEOUT + 2) + SETUP_SEC + READINESS_SEC + DIAGNOSTIC_SEC + CLEANUP_SEC))
[ "$WHOLE_LAB_SEC" -le 600 ] || fail "whole-lab budget exceeds 600s: ${WHOLE_LAB_SEC}s"
...
for i in $(seq 1 "$RUNS"); do
 ...
 ready=0; for _ in $(seq 1 200); do ss -H -lnt "sport = :${ports[0]}" | grep -q . && { ready=1; break; }; kill -0 "$spid" 2>/dev/null || break; sleep .05; done
