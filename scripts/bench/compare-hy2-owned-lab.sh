...
START_MS=$(date +%s%3N); GLOBAL_DEADLINE_MS=$((WORK_SEC * 1000)); DEADLINE_MS=$((START_MS + GLOBAL_DEADLINE_MS)); CLEANUP_DEADLINE_MS=$((START_MS + WHOLE_LAB_SEC * 1000)); cleanup_mode=0
...
ssh_bounded "$LAB_SSH_TARGET" "umask 077; mkdir '$remote'"; remote_started=1; tar -C "$run" -cf - neko-cli hysteria process-resource-sampler.py echo-payload.py echo-server.py payload.bin server.identity tls hy2-server.yaml | ssh_bounded "$LAB_SSH_TARGET" "tar -C '$remote' -xf -; chmod 700 '$remote/neko-cli' '$remote/hysteria'; : >'$remote/pids'"
...
for _ in $(seq 1 200); do
 ...
 ssh_bounded "$LAB_SSH_TARGET" "ss -H -lun | grep -E \":(${bind_authority}|0.0.0.0):${ports[1]} \"" >/dev/null 2>&1 && { hy2_ready=1; break; }
 sleep .05
done
...
for i in $(seq 1 "$RUNS"); do
 ...
 ready=0; for _ in $(seq 1 200); do ssh_bounded "$LAB_SSH_TARGET" "ss -H -lnt | grep -E \":(${bind_authority}|0.0.0.0):${ports[0]} \"" >/dev/null 2>&1 && { ready=1; break; }; kill -0 "$spid" 2>/dev/null || break; sleep .05; done
