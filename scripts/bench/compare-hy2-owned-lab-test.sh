...
# Arbitrary failure prose can classify a subsystem but never prove stage success.
for message in 'QUIC handshake failed' 'UDP timeout' 'no QUIC response' 'not authenticated' 'authentication failed' 'TLS handshake failed' 'TLS authentication failed' 'connected to server: false'; do
  printf '%s\n' "$message" >"$tmp/diagnostic.err"
  python3 "$root/scripts/bench/validate-hy2-owned-lab.py" make-sample --implementation hy2 --run 1 --return-code 9 --time "$tmp/neko-first.time" --resource "$tmp/neko-first.resource" --client-output /dev/null --client-diagnostics "$tmp/diagnostic.err" --diagnostic-bundle "$tmp/adversarial-private.json" --diagnostic-started-at 2026-09-03T00:00:00Z --diagnostic-ended-at 2026-09-03T00:00:01Z --diagnostic-stage client_started --bytes 1200 --payload-hash "$hash0" >"$tmp/adversarial.json"
  jq -e '.client_diagnostic.last_success_stage=="client_started" and .client_diagnostic.last_success_source=="harness"' "$tmp/adversarial.json" >/dev/null
done
# Only strict, typed, monotonic positive evidence can promote lifecycle state.
printf '%s\n' '{"schema":"nekomusume.hy2-stage-evidence.v1","events":[{"stage":"quic_udp","source":"capture","success":true},{"stage":"tls_authenticated","source":"capture","success":true}]}' >"$tmp/stage.json"
printf '%s\n' 'later client failure' >"$tmp/diagnostic.err"
python3 "$root/scripts/bench/validate-hy2-owned-lab.py" make-sample --implementation hy2 --run 1 --return-code 9 --time "$tmp/neko-first.time" --resource "$tmp/neko-first.resource" --client-output /dev/null --client-diagnostics "$tmp/diagnostic.err" --diagnostic-bundle "$tmp/promoted-private.json" --diagnostic-started-at 2026-09-03T00:00:00Z --diagnostic-ended-at 2026-09-03T00:00:01Z --diagnostic-stage client_started --diagnostic-stage-evidence "$tmp/stage.json" --bytes 1200 --payload-hash "$hash0" >"$tmp/promoted.json"
jq -e '.client_diagnostic.last_success_stage=="tls_authenticated" and .client_diagnostic.last_success_source=="capture"' "$tmp/promoted.json" >/dev/null
jq -e '.last_success_stage=="tls_authenticated" and .last_success_source=="capture"' "$tmp/promoted-private.json" >/dev/null
# One exact next-stage marker promotes monotonically, but cannot skip a stage.
printf '%s\n' '{"schema":"nekomusume.hy2-stage-evidence.v1","events":[{"stage":"client_started","source":"harness","success":true}]}' >"$tmp/stage.json"
python3 "$root/scripts/bench/validate-hy2-owned-lab.py" make-sample --implementation hy2 --run 1 --return-code 9 --time "$tmp/neko-first.time" --resource "$tmp/neko-first.resource" --client-output /dev/null --client-diagnostics "$tmp/diagnostic.err" --diagnostic-bundle "$tmp/server-baseline-private.json" --diagnostic-started-at 2026-09-03T00:00:00Z --diagnostic-ended-at 2026-09-03T00:00:01Z --diagnostic-stage server_bound --diagnostic-stage-evidence "$tmp/stage.json" --bytes 1200 --payload-hash "$hash0" >"$tmp/server-baseline.json"
jq -e '.client_diagnostic.last_success_stage=="client_started" and .client_diagnostic.last_success_source=="harness"' "$tmp/server-baseline.json" >/dev/null
for events in   '[{"stage":"tls_authenticated","source":"capture","success":true}]'   '[{"stage":"quic_udp","source":"capture","success":true},{"stage":"quic_udp","source":"capture","success":true}]'   '[{"stage":"tls_authenticated","source":"capture","success":true},{"stage":"quic_udp","source":"capture","success":true}]'   '[{"stage":"quic_udp","source":"capture","success":false}]'   '[{"stage":"quic_udp","source":"stderr","success":true}]'   '[{"stage":"quic_udp","source":"capture"}]'; do
  printf '{"schema":"nekomusume.hy2-stage-evidence.v1","events":%s}\n' "$events" >"$tmp/stage.json"
  python3 "$root/scripts/bench/validate-hy2-owned-lab.py" make-sample --implementation hy2 --run 1 --return-code 9 --time "$tmp/neko-first.time" --resource "$tmp/neko-first.resource" --client-output /dev/null --client-diagnostics "$tmp/diagnostic.err" --diagnostic-bundle "$tmp/rejected-private.json" --diagnostic-started-at 2026-09-03T00:00:00Z --diagnostic-ended-at 2026-09-03T00:00:01Z --diagnostic-stage client_started --diagnostic-stage-evidence "$tmp/stage.json" --bytes 1200 --payload-hash "$hash0" >"$tmp/rejected.json"
  jq -e '.client_diagnostic.last_success_stage=="client_started" and .client_diagnostic.last_success_source=="harness"' "$tmp/rejected.json" >/dev/null
done
printf '%s\n' 'not-json' >"$tmp/stage.json"
python3 "$root/scripts/bench/validate-hy2-owned-lab.py" make-sample --implementation hy2 --run 1 --return-code 9 --time "$tmp/neko-first.time" --resource "$tmp/neko-first.resource" --client-output /dev/null --client-diagnostics "$tmp/diagnostic.err" --diagnostic-bundle "$tmp/malformed-private.json" --diagnostic-started-at 2026-09-03T00:00:00Z --diagnostic-ended-at 2026-09-03T00:00:01Z --diagnostic-stage client_started --diagnostic-stage-evidence "$tmp/stage.json" --bytes 1200 --payload-hash "$hash0" >"$tmp/malformed.json"
jq -e '.client_diagnostic.last_success_stage=="client_started" and .client_diagnostic.last_success_source=="harness"' "$tmp/malformed.json" >/dev/null
