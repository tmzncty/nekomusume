...
run_one(){
  ...
  app=$(jq -r '.application_bytes // empty' "$raw" 2>/dev/null || true); wire=$(jq -c '.wire_bytes // null' "$raw" 2>/dev/null || echo null); fd=$(jq -r '.fd_count // empty' "$raw" 2>/dev/null || true)
  failure=0; [ "$rc" -eq 0 ] && [ "$exitcode" = 0 ] && [[ "$app" =~ ^[0-9]+$ ]] && [ "$app" -eq "$PAYLOAD_BYTES" ] && [[ "$fd" =~ ^[0-9]+$ ]] || failure=1
  jq -nc --arg implementation "$impl" --argjson run "$run" --argjson failures "$failure" --arg elapsed "$elapsed" --arg user "$user" --arg sys "$sys" --arg rss "$rss" --arg app "$app" --argjson wire "$wire" --arg fd "$fd" --argjson rc "$rc" '{name:($implementation+"-"+($run|tostring)),implementation:$implementation,run:$run,failures:$failures,elapsed_seconds:(if $elapsed=="" then null else ($elapsed|tonumber) end),cpu_user_seconds:(if $user=="" then null else ($user|tonumber) end),cpu_system_seconds:(if $sys=="" then null else ($sys|tonumber) end),rss_kib:(if $rss=="" then null else ($rss|tonumber) end),fd_count:(if $fd=="" then null else ($fd|tonumber) end),application_bytes:(if $app=="" then null else ($app|tonumber) end),wire_bytes:$wire,exit_code:$rc}' >>"$records"
}
